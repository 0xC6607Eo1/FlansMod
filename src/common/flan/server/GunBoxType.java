...
public class GunBoxType extends InfoType
{
		...
		try
		{
			...
			if(split[0].equals("AddGun"))
			{
				...
				for(int i = 0; i < (split.length - 2) / 2; i++)
				{
					...
					else gunParts[nextGun].add(getRecipeElement(split[i * 2 + 3], Integer.parseInt(split[i * 2 + 2]), 0));
				}
				nextGun++;
			}			
			if(split[0].equals("AddAmmo"))
			{
				...
				for(int i = 0; i < (split.length - 2) / 2; i++)
				{
					...
					else bulletParts[nextAmmo].add(getRecipeElement(split[i * 2 + 3], Integer.parseInt(split[i * 2 + 2]), 0));
				}
				nextAmmo++;
			}
			if(split[0].equals("AddAltAmmo") || split[0].equals("AddAlternateAmmo"))
			{
				...
				for(int i = 0; i < (split.length - 2) / 2; i++)
				{
					...
					else altBulletParts[nextAmmo - 1].add(getRecipeElement(split[i * 2 + 3], Integer.parseInt(split[i * 2 + 2]), 0));
				}
			}
		}
		catch (Exception e)
	...
	{
		if(mat.equals("wood")) 							return Material.wood;
		if(mat.equals("rock") || mat.equals("stone")) 	return Material.rock;
		if(mat.equals("iron")) 							return Material.iron;
		if(mat.equals("cloth")) 						return Material.cloth;
