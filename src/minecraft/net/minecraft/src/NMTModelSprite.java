	...
	protected void createSprite()
	{
		...
		for (int idz = 0; idz < layers; idz++)
		{
			...
			for (int idy = 0; idy < textureH; idy++)
			{
				...
				{
					int[] verts = new int[]
					{ idx + idy * texW + idz * texS, idx + 1 + idy * texW + idz * texS, idx + 1 + (idy + 1) * texW + idz * texS, idx + (idy + 1) * texW + idz * texS, idx + idy * texW + texS + idz * texS, idx + 1 + idy * texW + texS + idz * texS, idx + 1 + (idy + 1) * texW + texS + idz * texS, idx + (idy + 1) * texW + texS + idz * texS };
